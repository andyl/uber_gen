defmodule UberGen.Utils do
  @moduledoc false

  @doc """
  Gets the UberGen home.
  ...
  @doc """
  Gets possible location of global UberGen configuration.
  ...
  or the atom `:error`.
      ...
      iex> UberGen.Utils.parse_mfa("Foo.bar/1")
      {:ok, [Foo, :bar, 1]}
      iex> UberGen.Utils.parse_mfa(":foo.bar/1")
      {:ok, [:foo, :bar, 1]}
      iex> UberGen.Utils.parse_mfa(":foo.bar")
      {:ok, [:foo, :bar]}
      iex> UberGen.Utils.parse_mfa(":foo")
      {:ok, [:foo]}
      iex> UberGen.Utils.parse_mfa("Foo")
      {:ok, [Foo]}
      ...
      iex> UberGen.Utils.parse_mfa("Foo.")
      :error
      iex> UberGen.Utils.parse_mfa("Foo.bar.baz")
      :error
      iex> UberGen.Utils.parse_mfa("Foo.bar/2/2")
      :error
  ...
  ## Examples
      ...
      iex> UberGen.Utils.command_to_module("compile", UberGen.Tasks)
      {:module, UberGen.Tasks.Compile}
  ...
  # """
  # def compiling_n(1, ext), do: UberGen.shell().info("Compiling 1 file (.#{ext})")
  # def compiling_n(n, ext), do: UberGen.shell().info("Compiling #{n} files (.#{ext})")
  ...
  defp print_tree([node | nodes], depth, parent, seen, pretty?, callback) do
    ...
    if MapSet.member?(seen, key) do
      ...
      _info = if(info, do: " #{info}", else: "")
      # UberGen.shell().info("#{depth(pretty?, depth)}#{prefix(pretty?, depth, nodes)}#{name}#{info}")
  ...
  ## Examples
      ...
      iex> UberGen.Utils.module_name_to_command(UberGen.Tasks.Compile, 2)
      "compile"

      iex> UberGen.Utils.module_name_to_command("UberGen.Tasks.Compile.Elixir", 2)
      "compile.elixir"
  ...
  ## Examples
      ...
      iex> UberGen.Utils.command_to_module_name("compile.elixir")
      "Compile.Elixir"
  ...
  defp read_httpc(path) do
    ...
    headers = [{'user-agent', 'UberGen/#{System.version()}'}]
