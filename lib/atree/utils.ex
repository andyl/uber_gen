defmodule Atree.Utils do
  ...
  @doc """
  Gets possible location of global Atree configuration.
  ...
  or the atom `:error`.
      ...
      iex> Atree.Utils.parse_mfa("Foo.bar/1")
      {:ok, [Foo, :bar, 1]}
      iex> Atree.Utils.parse_mfa(":foo.bar/1")
      {:ok, [:foo, :bar, 1]}
      iex> Atree.Utils.parse_mfa(":foo.bar")
      {:ok, [:foo, :bar]}
      iex> Atree.Utils.parse_mfa(":foo")
      {:ok, [:foo]}
      iex> Atree.Utils.parse_mfa("Foo")
      {:ok, [Foo]}
      ...
      iex> Atree.Utils.parse_mfa("Foo.")
      :error
      iex> Atree.Utils.parse_mfa("Foo.bar.baz")
      :error
      iex> Atree.Utils.parse_mfa("Foo.bar/2/2")
      :error
  ...
  ## Examples
      ...
      iex> Atree.Utils.command_to_module("compile", Atree.Tasks)
      {:module, Atree.Tasks.Compile}
  ...
  # """
  # def compiling_n(1, ext), do: Atree.shell().info("Compiling 1 file (.#{ext})")
  # def compiling_n(n, ext), do: Atree.shell().info("Compiling #{n} files (.#{ext})")
  ...
  defp print_tree([node | nodes], depth, parent, seen, pretty?, callback) do
    ...
    if MapSet.member?(seen, key) do
      ...
      # Atree.shell().info("#{depth(pretty?, depth)}#{prefix(pretty?, depth, nodes)}#{name}#{info}")
  ...
  ## Examples
      ...
      iex> Atree.Utils.module_name_to_command(Atree.Tasks.Compile, 2)
      "compile"

      iex> Atree.Utils.module_name_to_command("Atree.Tasks.Compile.Elixir", 2)
      "compile.elixir"
  ...
  ## Examples
      ...
      iex> Atree.Utils.command_to_module_name("compile.elixir")
      "Compile.Elixir"
